    ...
    while ( MSGpkt )
    {
      switch ( MSGpkt->hdr.event )
      {
        ...
        case SPI_INCOMING_ZTOOL_PORT:
         	 // extern uint8 rxData[10];
          	//HalUARTWrite(0,rxData,len);
			//printf("%d %d",rxData[0],rxData[1]);
...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  ...
  switch ( pkt->clusterId )
  {
    case SAMPLEAPP_PERIODIC_CLUSTERID:
	  ...
	  //OLED_P8x16Str(0, 0, pkt->cmd.Data);//在OLED上显示温度
...
void SampleApp_SendPeriodicMessage( void )
{
  //uint8 data[] = "Hello Seven";
	...
	if (dht11Dat.ok) {
	    sprintf((char *)data, "%d%d%d%d", dht11Dat.temp,dht11Dat.humi,value,LED_STATUS);//将温度转换为字符串
